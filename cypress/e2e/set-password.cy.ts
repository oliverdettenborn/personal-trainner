describe('Set Password Flow', () => {
  beforeEach(() => {
    ...
    const keys = ['sb-auth-token', 'sb-127-0-0-1-auth-token', 'sb-localhost-auth-token'];
    keys.forEach(key => {
  ...
  it('should update password successfully', () => {
    ...
    cy.url().should('satisfy', (url: string) => url === Cypress.config().baseUrl + '/' || url.includes('/login'));
  });
});
