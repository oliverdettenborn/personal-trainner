...
export function useAssessment() {
  ...
  // --- ASSESSMENT ACTIONS ---
  const addAssessment = useCallback((studentId: string) => {
    const now = Date.now();
  ...
  const importFromJSON = useCallback((jsonData: any) => {
    try {
      ...
      setDb(prev => {
        ...
        let studentId = Object.entries(prev.students).find(([,s]) => s.name.toLowerCase() === name.toLowerCase())?.[0];
        
        const newStudents = { ...prev.students };
        if (!studentId) {
          studentId = 's_' + Date.now();
          newStudents[studentId] = { id: studentId, name, createdAt: Date.now() };
        }
