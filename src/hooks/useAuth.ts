...
export function useAuth() {
  ...
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, s) => {
      ...
      if (event === 'SIGNED_IN' && hasAuthCallback && (initialUrlType === 'invite' || initialUrlType === 'recovery')) {
      ...
      if (event === 'SIGNED_IN' || event === 'SIGNED_OUT' || event === 'PASSWORD_RECOVERY') setLoading(false);
    });
